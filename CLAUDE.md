...
3. **마크다운 파서 (`markdown.js`)**
   ...
   - **경로 규칙**: 이미지(`![alt](경로)`)의 상대 경로는 `posts/` 폴더 기준으로 자동 해석된다(글 파일과 같은 폴더에 이미지를 두면 됨). 링크(`[글자](경로)`)는 그대로 사이트 루트 기준이다 — `[다른 글](post.html?slug=...)`처럼 다른 페이지를 가리키는 데 쓰기 때문에 posts/ 기준으로 바꾸지 않는다. `javascript:`/`data:` 등 안전하지 않은 URL 스킴은 링크·이미지 모두 무시된다.
