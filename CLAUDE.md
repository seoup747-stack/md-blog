...
- **빌드 도구 금지**: webpack/vite/npm 빌드 스텝 없이, 브라우저에서 HTML 파일을 열거나 정적 서버로 서빙하면 바로 동작해야 한다.
...
- 사용자가 `posts/` 폴더에 `.md` 파일을 넣으면, 별도 빌드 없이 브라우저에서 목록 페이지와 개별 글 페이지로 볼 수 있어야 한다.
- 디자인은 **깔끔하고 가독성 중심**이어야 한다 (넓은 줄 간격, 적절한 본문 폭 max-width, 읽기 편한 폰트 크기).
- **다크 모드**를 지원한다 (시스템 설정 자동 감지 `prefers-color-scheme` + 수동 토글 버튼, 선택은 `localStorage`에 저장).
- **모바일 반응형**: 작은 화면에서도 레이아웃이 깨지지 않고 터치 UI로 잘 보여야 한다.
...
├── post.html           # 개별 글 상세 페이지 (쿼리 파라미터나 해시로 어떤 글인지 식별)
...
├── posts/
│   ├── manifest.json   # 글 목록 메타데이터 (title, date, slug, file, summary, tags)
│   └── *.md             # 실제 글 원본
└── tools/
...
브라우저 JS는 보안상 서버 없이 폴더 목록을 읽을 수 없다. 따라서 `posts/` 안에 어떤 글이 있는지 알려주는 `manifest.json`을 두고, 새 글을 추가할 때마다 이 파일에 항목을 추가하는 방식으로 관리한다. (예: `{ "slug": "hello-world", "title": "...", "date": "2026-08-01", "file": "hello-world.md" }`)
...
4. **코드 문법 강조 (`highlight.js`)**
   ...
   - 지원 언어: `js`/`ts`, `python`, `bash`/`sh`, `json`, `css`, `html`. 새 언어를 추가하려면 `define([...], "...")` 호출을 하나 더 추가하면 된다.
